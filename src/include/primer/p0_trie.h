...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (!child || key_char != child->key_char_) {
      return nullptr;
    }
  ...
  void RemoveChildNode(char key_char) {
    auto i = children_.find(key_char);
    if (i != children_.end()) {
      children_.erase(i);
    }
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (size_t i = 0; i < key.size(); ++i) {
      ...
      auto child = (**node).GetChildNode(key[i]);
      if (i == key.size() - 1) {
        if ((**child).IsEndNode()) {
          latch_.WUnlock();
          return false;
        }
        std::unique_ptr<TrieNode> new_child = std::make_unique<TrieNodeWithValue<T>>(std::move(**child), value);
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.WLock();
    auto *node = &root_;
    std::stack<std::unique_ptr<TrieNode> *> s;
    for (char i : key) {
      s.push(node);
      if (!node->get()->HasChild(i)) {
        latch_.WUnlock();
        return false;
      }
    ...
    while (s.size() > 1) {
      ...
      if (!node->get()->IsEndNode() && !node->get()->HasChildren()) {
        prt->get()->RemoveChildNode(node->get()->GetKeyChar());
      }
    }
    latch_.WUnlock();
    return true;
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto n = dynamic_cast<TrieNodeWithValue<T> *>(node->get());
    if (n == nullptr) {
      latch_.RUnlock();
      return {};
    }
